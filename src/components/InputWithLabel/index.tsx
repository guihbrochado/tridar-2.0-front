...
export function InputWithLabel({ label, placeholder, type, name, value, children }: PropsInputLabel) {
    return (
        <div className="flex-1 w-full justify-center items-center gap-3">
            ...
            <div className="bg-gray-dark flex justify-center items-center rounded-md px-3 mt-2">
                {children}
                <Input className="bg-gray-dark border-none text-white" name={name} value={value} type={type} placeholder={placeholder} />
            </div>
        </div>
    )
}
