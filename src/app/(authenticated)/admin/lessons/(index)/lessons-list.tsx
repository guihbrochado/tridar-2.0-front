...
export default function LessonsList(data: any) {
    ...
    async function deleteModule() {
        ...
        try {
            const response = await fetch(`http://homologacao.tridar.log.br/lessons/${selectedId}`, {
                method: 'DELETE',
                headers: {
                    Authorization: token
                }
            ...
            toast.success("Falha ao deletar a aula!");
                ...
                data.data.map((item: any, key: number) => (
                    ...
                    >
                        ...
                        <div className="hidden items-center justify-center gap-3 p-2.5 sm:flex xl:p-5">
                            <Link href={`/admin/lessons/see/${item.ID}`}>
                                <Icon.Eye size={32} weight="fill" />
                            </Link>
                            <Link href={`/admin/lessons/edit/${item.ID}`}>
                                            ...
                                            <div className="bg-gray-900 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                                <div className="sm:flex sm:items-start">
                                                    ...
                                                    <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                                        <h3 className="text-base font-semibold leading-6 text-white" id="modal-title">Deletar Aula</h3>
                                                        <div className="mt-2">
                                                            <p className="text-sm text-gray-400">Tem certeza que deseja deletar este aula?</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
