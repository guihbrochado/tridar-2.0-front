...
export default function LivesList(data: any) {
    ...
    async function deleteModule() {
        ...
        try {
            const response = await fetch(`http://homologacao.tridar.log.br/lives/${selectedId}`, {
                method: 'DELETE',
                headers: {
                    Authorization: token
                }
                ...
                data.data.map((item: any, key: number) => (
                    ...
                    >
                        ...
                        <div className="hidden items-center justify-center gap-3 p-2.5 sm:flex xl:p-5">
                            <Link href={`/admin/lives/see/${item.ID}`}>
                                <Icon.Eye size={32} weight="fill" />
                            </Link>
                            <Link href={`/admin/lives/edit/${item.ID}`}>
